import styles from './RankDropdown.module.css';
import arrowDown from '../../assets/ic_toggle.svg';
import { useEffect, useRef, useState } from 'react';
//import { useSort } from '../../contexts/SortContext';

export default function RankDropdown({ setSortOptionBy }) {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedOption, setSelectedOption] = useState('revenue_desc');
  ...
  const handleOptionClick = (value) => {
    ...
    setSortOptionBy(value); // 부모 컴포넌트로 선택된 옵션 전달
    setIsOpen(false);
  };
